...
{
    public class TestChecksumService : IDisposable
    {
        ...
        [Fact]
        public void Test_GetAlgorithmCode()
        {
            {
                var algorithm = ChecksumAlgorithm.SHA256;
                var code = _checksumService.GetAlgorithmCode(algorithm);
                Assert.Equal("sha256", code);
            }
            ...
            {
                    var file = Path.Combine(_testDir, "hello.txt");
                    Assert.False(_checksumService.CompareChecksum(file, "d41d8cd98f00b204e9800998ecf8427e", ChecksumAlgorithm.MD5));
            ...
            {
                var file = Path.Combine(_testDir, "hello.txt");
                Assert.True(_checksumService.CompareChecksum(file, variant, ChecksumAlgorithm.MD5));   
            }
            ...
            {
                var file = Path.Combine(_testDir, "hello.txt");
                Assert.Throws<ArgumentException>(() => _checksumService.CompareChecksum(file, "8b1a9953c4611296a827abf8c47804d7", ChecksumAlgorithm.SHA1));
            }
            ...
            {
                var file = Path.Combine(_testDir, "hello.txt");
                Assert.Throws<ArgumentNullException>(() => _checksumService.CompareChecksum(file, checksum!, ChecksumAlgorithm.SHA512));
            }
